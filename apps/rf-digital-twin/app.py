...
# Passed through to the render job, which gets no Lakebase resource binding of
# its own. Both are injected by the bundle; the defaults keep local dev working.
LAKEBASE_INSTANCE = os.environ.get("LAKEBASE_INSTANCE", "rf-digital-twin-pg")
LAKEBASE_DATABASE = os.environ.get("PGDATABASE", "rf_digital_twin")
...
def _submit_databricks_job(config_hash: str, scene_cfg: dict, cells: list[dict]) -> int:
    ...
    run = w.jobs.run_now(
        ...
        job_parameters={
            ...
            # The job has no Lakebase resource binding, so tell it where to write.
