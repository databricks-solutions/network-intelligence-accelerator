...
# MAGIC
# MAGIC 1. Creates the `<catalog>.<schema>` schema in Unity Catalog (defaults to
# MAGIC    `main.sionna_rf_data`; override via the widgets in section 1).
...
# MAGIC > - `USE CATALOG` + `CREATE SCHEMA` on your target catalog
...
# When run as a bundle job these arrive as job parameters; when run
# interactively the defaults below apply. Edit the defaults to match your
# workspace, or pass overrides via the widgets.
dbutils.widgets.text("catalog", "main", "Unity Catalog catalog")
dbutils.widgets.text("schema", "sionna_rf_data", "Unity Catalog schema")
dbutils.widgets.text("lakebase_instance", "rf-digital-twin-pg", "Lakebase instance")
dbutils.widgets.text("lakebase_database", "rf_digital_twin", "Lakebase database")
...
LAKEBASE_DB       = dbutils.widgets.get("lakebase_database")

print(f"UC target        : {UC_FQN}")
print(f"Lakebase instance: {LAKEBASE_INSTANCE} / db={LAKEBASE_DB}")
...
# MAGIC %md
# MAGIC ## 11. Next step — grant the app service principal Postgres access
# MAGIC
# MAGIC The Lakebase cache is populated. The rest of the deployment is handled by
# MAGIC the Databricks Asset Bundle at the repo root:
# MAGIC
# MAGIC ```bash
# MAGIC databricks bundle deploy -t dev
# MAGIC ```
# MAGIC
# MAGIC That creates both apps, the GPU render job, and the Lakebase resource
# MAGIC binding (which injects `PGHOST` / `PGPORT` / `PGDATABASE` / `PGUSER` and
# MAGIC provisions a Postgres role for the app's service principal). The app mints
# MAGIC its own short-lived OAuth token, so **no `PGPASSWORD` secret is needed.**
# MAGIC
# MAGIC The one step the bundle can't do: the tables above are owned by *your*
# MAGIC user, so the app's service-principal role can connect but not read them.
# MAGIC Run the next cell **after** `bundle deploy` to grant access.
# MAGIC
# MAGIC See [`docs/deployment.md`](../../../docs/deployment.md) for the full guide.

# COMMAND ----------

# Run this AFTER `databricks bundle deploy` has created the app.
#
# Grants are to PUBLIC for demo simplicity — every Postgres role on this
# instance gets read/write on these tables. For anything beyond a demo, replace
...
print("Granted Postgres access on public schema.")
...
print(f"  Lakebase database : {LAKEBASE_DB}")
print()
...
print()
print("Now deploy the apps + render job:")
