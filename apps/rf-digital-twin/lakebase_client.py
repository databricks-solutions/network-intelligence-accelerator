...
# Default Lakebase instance / database names. Both are set for real by the
# bundle (see databricks.yml) via LAKEBASE_INSTANCE / PGDATABASE; these
# fallbacks only matter for local dev.
_DEFAULT_INSTANCE_NAME = "rf-digital-twin-pg"
_DEFAULT_DATABASE_NAME = "rf_digital_twin"
...
def _conn_kwargs() -> dict:
    ...
    Resolution order:
      ...
      PGDATABASE — env var, else _DEFAULT_DATABASE_NAME.
    ...
    return dict(
        ...
        dbname=_pick("PGDATABASE", "LAKEBASE_DATABASE", default=_DEFAULT_DATABASE_NAME),
        user=user,
        password=password,
